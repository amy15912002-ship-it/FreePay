...
| 真實零值 `0` | 該欄位實際數值為 0（如「未領過 Pay」「已完全領完剩餘 0 單位」），用阿拉伯數字 `0`，與其他實數一致 | `0`、`台幣 0` |
| 無參考意義 placeholder `-` | 該欄位**有資料但對使用者無參考意義**（如「已領完批次的未實現報酬率」「不適用情境的指標」）或**真的無資料**，使用 hyphen-minus `-`（U+002D），禁混用 em dash `—` 或 en dash `–` | `-`（✗ `—`、`–`、空字串） |

> **`0` vs `-` 分工原則**：`0` 是「真的就是 0 元 / 0 單位」的事實陳述；`-` 是「這個數字算出來也沒用，不要看」的設計判斷。同一欄位混用須避免；若一個語意可同時對應 0 與 -，以 spec 明確定義為準。
